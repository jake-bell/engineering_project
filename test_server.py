...
INSERTION_POINTS = [11 + PAST_DATAPOINTS, 12*6-1 + PAST_DATAPOINTS, 12*24-1 + PAST_DATAPOINTS, 12*48-1 + PAST_DATAPOINTS, 12*72-1 + PAST_DATAPOINTS];

# X values for plotting past datapoints
x = np.arange(0, PAST_DATAPOINTS, 1)
# X values for plotting future datapoints.
x_future = np.arange(0, PAST_DATAPOINTS + FUTURE_DATAPOINTS, 1)
...
PAST_DATA_FILE = "test_server_data/%s_past.np"
FUTURE_DATA_FILE = "test_server_data/%s_future.np"
LOCATIONS_FILE = "test_server_data/locations.json"
...
def load_data():
	...
	for location in locations:
		...
		if os.path.exists(FUTURE_DATA_FILE % location):
			...
			data_dict[location]['future'] = np.zeros(FUTURE_DATAPOINTS + PAST_DATAPOINTS)
